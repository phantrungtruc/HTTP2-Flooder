...
def proxylist(): # funzione per la creazione della proxylist
	...
	print("  Các IP hiện tại trong danh sách proxy: %s" % (len(open("proxy.txt").readlines()))) # per vedere quante lines (e quindi quanti proxy) ci sono nel file
...
def main(): # funzione effettiva del programma.
	try:
		...
		proxyget2(url)
		print("  Các IP hiện tại trong danh sách proxy: %s" % (len(open("proxy.txt").readlines()))) # printa la lunghezza attuale del file, che sarebbe il numero di proxy
		...
		print("  Các IP hiện tại trong danh sách proxy: %s" % (len(open("proxy.txt").readlines())))

		print ("\n  Đang tải xuống từ nhiều máy nhân bản khác nhau ...")
		for position, url in enumerate(nurls):
			proxyget(url)
			print("  Đã hoàn thành tải xuống: (%s/%s)\n  Các IP hiện tại trong danh sách proxy: %s" % (position+1, len(nurls), len(open("proxy.txt").readlines())))
		...
		print("\n")
		while True:
			choice = input("\n  Bạn có muốn kiểm tra proxy không? [Y/n]> ") # scelta di quello che vuole l'utente
...
if __name__ == '__main__':
	...
	while True:
		choice = input("\n  Bạn có muốn tải xuống proxy không? [Y/n]> ")
